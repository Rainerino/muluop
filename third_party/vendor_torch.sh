...
WHEEL_DIR="third_party/python/torch"
...
uv pip compile third_party/requirements.torch.in \
  -o third_party/requirements.torch.txt \
...
uv run pip download \
    -r third_party/requirements.torch.txt \
    --dest "$WHEEL_DIR" \
    --no-deps \
    --require-hashes
...
uv run python third_party/generate.py --target-dir "$WHEEL_DIR"
